...
async def test_email_read_and_user_isolation() -> None:
    ...
    first = await executor.execute(connector_id="local_email", operation="get_message", raw_arguments={"message_id": "phoenix-budget"}, context=context(1))
    second = await executor.execute(connector_id="local_email", operation="get_message", raw_arguments={"message_id": "phoenix-budget"}, context=context(2))
...
async def test_write_and_destructive_operations_require_approval() -> None:
    executor = ConnectorExecutor()
    email = await executor.execute(connector_id="local_email", operation="send_email", raw_arguments={"to": "manager@local.test", "subject": "Ready", "body": "Report is ready"}, context=context())
    calendar = await executor.execute(connector_id="local_calendar", operation="delete_event", raw_arguments={"event_id": "phoenix-standup"}, context=context())
...
async def test_unknown_connector_operation_and_input_are_safe() -> None:
    executor = ConnectorExecutor()
    unknown = await executor.execute(connector_id="not_real", operation="read", raw_arguments={}, context=context())
    operation = await executor.execute(connector_id="mcp", operation="shell", raw_arguments={}, context=context())
    invalid = await executor.execute(connector_id="local_email", operation="get_message", raw_arguments={}, context=context())
...
async def test_workspace_blocks_traversal_absolute_and_secrets(tmp_path: Path) -> None:
    ...
    for requested in ("../../.env", "C:/Windows/system.ini", ".env"):
        result = await executor.execute(connector_id="workspace", operation="read_text_file", raw_arguments={"path": requested}, context=context())
...
async def test_mcp_discovery_execution_and_secret_redaction() -> None:
    ...
    result = await ConnectorExecutor().execute(connector_id="mcp", operation="echo", raw_arguments={"text": "safe", "access_token": "never-return"}, context=context())
...
async def test_github_and_calendar_reads_are_scoped() -> None:
    executor = ConnectorExecutor()
    issues = await executor.execute(connector_id="github_mock", operation="list_issues", raw_arguments={}, context=context(4))
    events = await executor.execute(connector_id="local_calendar", operation="list_events", raw_arguments={}, context=context(5))
