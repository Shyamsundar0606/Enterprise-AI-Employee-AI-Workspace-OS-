...
class GitHubMockConnector(BaseConnector):
    ...
    def capabilities(self) -> tuple[ConnectorCapability, ...]:
        return (
            ...
            ConnectorCapability(
                ...
                access_type=AccessType.READ,
                input_schema=IssueInput.model_json_schema(),
            ),
            ConnectorCapability(
                name="create_issue",
    ...
    def input_model(self, operation: str) -> type[BaseModel]:
        models = {
            "list_repositories": EmptyInput,
            "list_issues": EmptyInput,
            "get_issue": IssueInput,
            "create_issue": EmptyInput,
            "close_issue": IssueInput,
        }
    ...
    async def execute(
        ...
        issues = [
            {
                "number": 1,
                "title": "Harden integration policy",
                "state": "open",
        ...
        if operation == "get_issue":
            issue = next(
                (item for item in issues if item["number"] == input_data.issue_number), None
            )
            if issue is None:
